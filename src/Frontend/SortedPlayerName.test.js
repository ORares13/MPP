...
describe('SortedPlayerList', () => {
    it('renders sorted player list by Name', () => {
        const playerRepoMock = new PlayerRepo();
        const playerServiceMock = new PlayerService(playerRepoMock);
        const { getByText } = render(<SortedPlayerName playerService={playerServiceMock} />);
        ...
        expect(playerRows[0].textContent).toContain('Jayson Tatum');
        expect(playerRows[1].textContent).toContain('LeBron James');
        expect(playerRows[2].textContent).toContain('Luka Doncic');
    });
});
